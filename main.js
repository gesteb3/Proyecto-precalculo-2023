...
function calpp() {
    ...
    const peInput = document.getElementById('pe1').value;
    const xInput = document.getElementById('x1').value;
    const yInput = document.getElementById('y1').value;

    // Función para convertir una entrada en número (si es un número válido)
    const parseNumber = (input) => {
        if (/^-?\d+\/\d+$/.test(input)) {
            // Es una fracción con o sin signo negativo, divídela
            const [numerador, denominador] = input.split('/');
            return parseFloat(numerador) / parseFloat(denominador);
        } else if (/^-?\d+(\.\d+)?$/.test(input)) {
            // Es un número decimal con o sin signo negativo
            return parseFloat(input);
        }
        return NaN; // Valor no válido
    };

    const pe1 = parseNumber(peInput);
    const x1 = parseNumber(xInput);
    const y1 = parseNumber(yInput);

    // Verifica si los valores son números válidos
    if (!isNaN(pe1) && !isNaN(x1) && !isNaN(y1)) {
        // Formatea los números negativos correctamente
        const signX = x1 < 0 ? "-" : "+";
        const signY = y1 < 0 ? "-" : "+";
        const signC = y1 - pe1 * x1 < 0 ? "-" : "+";
        // Cuando no desees mostrar el signo "+"

// Paso 2: Crea una cadena con el procedimiento y el resultado
const procedimiento =
  ` <br>
  FÓRMULA: Y - Y1 = m(X - X1) <br><br>
  1. Y ${y1 < 0 ? `- ${Math.abs(y1)}` : `+ ${y1}`} = ${pe1 < 0 ? `${pe1}X - ${Math.abs(pe1 * x1)}` : `${pe1}X + ${pe1 * x1}`}<br><br>
  2. Y ${y1 < 0 ? `- ${Math.abs(y1)}` : `+ ${y1}`} = ${pe1 < 0 ? `${pe1}X - ${Math.abs(pe1 * x1 + y1)}` : `${pe1}X + ${pe1 * x1 + y1}`}<br><br>
  3. Y ${y1 < 0 ? `- ${Math.abs(y1)}` : `+ ${y1}`} = ${pe1 < 0 ? `${pe1}X - ${Math.abs(pe1 * x1 + y1)}` : `${pe1}X + ${pe1 * x1 + y1}`}<br><br>
  4. Y = ${pe1}X ${pe1 * x1 + y1 < 0 ? `- ${Math.abs(pe1 * x1 + y1)}` : `+ ${pe1 * x1 + y1}`}<br><br>`;

        // Mostrar el procedimiento y el resultado
        const elementoRespuesta = document.getElementById('respuesta');
        elementoRespuesta.innerHTML = 'Procedimiento:<br>' + procedimiento + '\n\nLa ecuación de la recta en forma punto-pendiente es: ' + `Y = ${pe1}X ${signC} ${Math.abs(y1 - pe1 * x1)}<br><br>`;
    } else {
        const elementoRespuesta = document.getElementById('respuesta');
        elementoRespuesta.innerHTML = 'Por favor, ingrese números válidos o divisiones en los campos de entrada.';
    }
